...
public enum QueueKind {
    ...
    public func toKind() -> dispatch_queue_attr_t? {
        switch self {
        case .Concurrent:
...
public class Queue {
    ...
    public init(_ label:String, _ kind:QueueKind) {
    ...
    public func getContext<T:AnyObject>() -> T? {
        return self.context as? T
    }
    
    public func getSpecific<T:AnyObject>(key:String) -> T? {
        return getObjectAtPointer(dispatch_queue_get_specific(_queue, ptrForKey(key)))
    }
    
    public func setSpecific<T:AnyObject>(key:String, value:T?) {
    ...
    public func ptrForKey(key:String)->UnsafePointer<()> {
        var result = _keys[key]
        if !result {
            result = result !! UnsafePointer<Int>.alloc(1)
            _keys[key] = result
        }
        return result!
    }
    
    public func async(block: @auto_closure ()->()) -> TaskVoid {
        dispatch_async(_queue, block)
        return TaskVoid()
    }
    
    public func async<T>(block: @auto_closure ()->T) -> Task<T> {
